// frontend/src/pages/UploadQuiz.js
...
export default function UploadQuiz() {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("assessment_type", "quiz");
    formData.append("file", file);

    try {
      setLoading(true);
      const data = await uploadAssessment(formData);
      setMessage(data.message || "Quiz data uploaded successfully.");
    } catch (err) {
      console.error(err);
      setMessage("Error uploading quiz file. Check console / backend logs.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">
            Upload topic-wise quiz scores using CSV or Excel
          </p>
        </div>
